...
Read the extended walkthrough in [`wiki/presentation/app-overview.md`](wiki/presentation/app-overview.md).
...
2. **Methodologies**:
   ...
   - **BEM Methodology**: follow BEM naming in `person/styles.css` and any new presentation markup.
   - Prefer modifier classes (e.g. `question-card--core`) instead of boolean attributes to toggle UI states.
3. The browser bootstrap lives in `presentation/main.js`; load `person/index.html` directly in a modern browser to run the app.
4. Internationalised strings belong to `i18n/translations.js`; use the translator service for all user-facing copy.
5. Add documentation for every new file or module under `person/wiki/`.
...
There are currently no automated tests for the `person` module. When introducing new features, add browser-friendly tests (e.g. Playwright or headless Mocha) and document them in this section.
